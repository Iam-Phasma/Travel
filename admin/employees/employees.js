    ...
    const renderEmployeeList = async (filteredData = null) => {
        try {
            ...
            employeeListContainer.innerHTML = allEmployeesData.map(emp => {
                ...
                const toggleLabel = emp.is_active === false ? 'Activate employee' : 'Deactivate employee';
                return `
                <div class="employee-item${inactiveClass}">
                    ...
                    <div class="employee-item-actions">
                        ...
                        <button class="edit-employee-btn icon-btn" data-id="${emp.id}" data-name="${emp.name}" aria-label="Edit employee" title="Edit employee">
                            <svg viewBox="0 0 24 24" role="img" aria-hidden="true" focusable="false">
